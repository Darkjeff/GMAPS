...
/**
 * \file    gmaps/admin/setup.php
 * \ingroup gmaps
 * \brief   Gmaps setup page.
 */
...
require_once '../lib/gmaps.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "gmaps@gmaps"));
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/gmaps/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
} elseif ($action == 'setmod')
{
	...
	$tmpobjectkey = GETPOST('object');
	$constforval = 'GMAPS_'.strtoupper($tmpobjectkey)."_ADDON";
...
$page_name = "GmapsSetup";
llxHeader('', $langs->trans($page_name));

// Subheader
$linkback = '<a href="'.($backtopage ? $backtopage : DOL_URL_ROOT.'/admin/modules.php?restore_lastsearch_values=1').'">'.$langs->trans("BackToModuleList").'</a>';

print load_fiche_titre($langs->trans($page_name), $linkback, 'object_gmaps@gmaps');

// Configuration header
$head = gmapsAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', '', -1, "gmaps@gmaps");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("GmapsSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'GMAPS_'.strtoupper($myTmpObjectKey).'_ADDON';

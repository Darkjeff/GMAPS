 ...
 *        \ingroup    gmaps
 *        \brief      List page for gmapsActivity
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'gmapsActivitylist'; // To manage different context of search
...
$hookmanager->initHooks(array('gmapsActivitylist')); // Note that conf->hooks_modules contains array
...
$permissiontoadd = $user->rights->gmaps->gmapsActivity->write;
$permissiontodelete = $user->rights->gmaps->gmapsActivity->delete;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'GmapsActivity';
	$objectlabel = 'GmapsActivity';
		...
		if (!empty($toselect)) {
			foreach ($toselect as $kay => $val) {
				...
				if ($result < 0) {
					...
					$$result_Upd = $act->setStatut(GmapsActivity::STATUS_CANCELED, $val);
					if ($result_Upd < 0) {
						setEventMessages($act->error, $act->errors, 'errors');
					}
				}
...
//$help_url="EN:Module_GmapsActivity|FR:Module_GmapsActivity_FR|ES:Módulo_GmapsActivity";
$help_url = '';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("GmapsActivitys"));
$morejs = array();
$morecss = array();
...
$topicmail = "SendGmapsActivityRef";
$modelmail = "gmapsActivity";
$objecttmp = new GmapsActivity($db);
